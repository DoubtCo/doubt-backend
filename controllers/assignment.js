...
exports.getAllAssignments = async (req, res, next) => {
    try {
        if (req.query.want == "all") {
            const foundAssignment = await Assignment.find({ college: req.params.college, sem: req.params.semester, branch: req.params.branch });
            res.send(foundAssignment);
        }
        else {

            let foundAssignment = await Assignment.findOne({ college: req.params.college, sem: req.params.semester, branch: req.params.branch })
                .populate("answers.assignmentId")
                .populate({ path: "answers.assignmentId", populate: { path: "createdBy",select:['name'] } })
            let obj;
